...
test.describe('QuietSlack Smoke & Core UI Suite', () => {
  ...
  test('can switch between channels and search modal', async ({ page }) => {
    ...
    // Open Search modal via header trigger
  ...
  test('can toggle huddle voice interface', async ({ page }) => {
    // Huddle button in channel header
    const huddleBtn = page.locator('#channel-huddle-btn');
    await expect(huddleBtn).toBeVisible();
    await huddleBtn.click();

    // Huddle floating dock appears when active
    await expect(page.locator('#huddle-floating-dock')).toBeVisible({ timeout: 10000 });
