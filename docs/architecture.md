...
Huddle join/leave/update signals carry mute, camera, and screen-share flags alongside the user identity. Listen-only joins (mic denied) still emit `join` so teammates always converge on the same channel-scoped huddle room. Starting a huddle writes a CRDT `huddle_started` system message into the channel timeline (and `huddle_ended` on leave); remote peers surface that as an in-app toast, browser notification, and optional Join Huddle action. Multi-browser Playwright coverage drives a shared local Nostr relay (`tests/e2e/localNostrRelay.mjs`) so isolated browser contexts truly peer-connect rather than relying on per-context BroadcastChannel mocks.

The app root intentionally omits React `StrictMode`. Trystero keeps a process-wide Nostr relay manager; StrictMode’s effect double-mount runs `room.leave()` (async unsubscribe) then immediately `joinRoom` again, which reuses stale sockets and drops SDP offers before `setRemoteDescription`. Production already mounts once — matching that lifecycle keeps huddles and CRDT mesh reliable. `P2PNetworkManager.leaveWorkspace` is still deferred one macrotask and guarded by `sessionEpoch` so any fast remount cannot tear down a newer room. E2E can pin relays via `window.__OPENSLACK_E2E_RELAYS`.
...
- **Standalone Mode Support**: Web App Manifest is generated by `vite-plugin-pwa` with a release-scoped `id`; registration lives in `src/main.tsx` via `virtual:pwa-register` (browser tab and installed PWA share the same worker).
- **Auto-Updates**: `registerType: 'autoUpdate'` with Workbox `skipWaiting` + `clientsClaim`. `onNeedRefresh` immediately activates the waiting worker and reloads open clients onto the new release. Visibility/online/60s polling also call `registration.update()`.
- **Release cache busting**:
  - Vite emits content-hashed JS/CSS (`assets/[name]-[hash].{js,css}`).
  - Each build injects `__APP_VERSION__` + `__BUILD_ID__` (Git SHA or timestamp) into the client.
  - Workbox precache revisions include HTML/assets; `cleanupOutdatedCaches` drops prior releases.
  - Navigation requests use `NetworkFirst` so the shell is not sticky across deploys; hashed assets use `CacheFirst` under a build-scoped cache name.
- **Install Touchpoints**:
