...
`P2PNetworkManager.leaveWorkspace` is intentionally deferred one macrotask and guarded by a monotonic `sessionEpoch`. React StrictMode remounts call leave then immediately re-join; without epoch gating, Trystero’s async room unsubscribe races the new join and leaves peers stuck with local offers and no remote SDP. `joinWorkspace` cancels any pending leave, bumps the epoch, and tears down synchronously before creating the next room. E2E can pin relays via `window.__OPENSLACK_E2E_RELAYS`.
